...
test('H1: the full fixture site clones to a temp dir, and the agent boots against it and serves pages with no steps beyond config (portability drill)', async () => {
  ...
  try {
    ...
    const { config, themeTemplates, layouts, engine } = bootSite(clonedSiteRoot);
    ...
    const aboutHtml = await renderPage(config, themeTemplates, layouts, engine, 'pages/about.json', 'public');
    assert.ok(aboutHtml.includes('About Us'));
    assert.ok(aboutHtml.includes('Meet the team'));

    const teamHtml = await renderPage(config, themeTemplates, layouts, engine, 'pages/about/team.json', 'public');
    assert.ok(teamHtml.includes('Our Team'));

    // Public mode still honours published: false against the cloned copy.
    await assert.rejects(renderPage(config, themeTemplates, layouts, engine, 'pages/hidden.json', 'public'));
