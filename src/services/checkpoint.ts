...
async function checkpointJob(config: SiteConfig, author: CommitAuthor): Promise<'committed' | 'clean'> {
  ...
  const status = execFileSync('git', ['status', '--porcelain', '--', DRAFTS_PATHSPEC], {
    cwd: config.siteRoot,
  }).toString('utf-8');
  ...
  commitPaths(config.siteRoot, [DRAFTS_PATHSPEC], CHECKPOINT_MESSAGE, author);
  return 'committed';
}
