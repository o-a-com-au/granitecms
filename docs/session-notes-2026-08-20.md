...
- New `docs/guide-hosting.md` - requirements, the current pre-npm-publish `npm pack` + `file:` install step, `site.config.json` reference, all three hosting shapes, and the tunnel feature.
...
- `@oa/cms-agent` is still unpublished (`package.json`'s `"private": true`) - every site still needs the manual `npm pack` + `file:` dependency step documented in `docs/guide-hosting.md`. Publishing is a deliberate future human action, not something to do as a side effect of other work.
