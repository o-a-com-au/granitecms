...
2. **A settings schema**, embedded in a `{% schema %} ... {% endschema %}` block. This is never a live Liquid tag - it is stripped out by the CMS before the file is rendered, so it can be placed anywhere in the file (convention: at the end). The content inside must be a single valid JSON object using *only* the JSON Schema draft-07 keywords listed below - never the full spec. Follow the existing convention exactly: `"type": "object"`, `"additionalProperties": false`, an explicit `"required"` array, and a `"properties"` object with a JSON Schema type for each setting (`string`, `integer`, `boolean`, `array`, etc., with constraints like `minLength`/`minimum`/`maximum`/`enum` as appropriate). There is no separate "settings type" system for most fields - a `select` is just a `string` with an `"enum"`, a checkbox is just `boolean`, and so on. The one deliberate exception is the `"format"` keyword (itself a real, if unvalidated, JSON Schema keyword), which the admin reads to choose a richer widget for the shapes below:

   **Supported keywords, and only these**: `type`, `properties`, `required`, `additionalProperties`, `default`, `minLength`, `maxLength`, `minimum`, `maximum`, `pattern`, `enum`, `items`, `minItems`, `maxItems`, plus the custom `format`/`title`/`description`/`allowedBlocks`/`api`/`swatches`/`step`/`unit` keywords covered below. **Never `$ref`, `$defs`, `definitions`, `allOf`, `anyOf`, `oneOf`, `not`, or `if`/`then`/`else`.** `requiredFieldsHaveValidDefaults` (the check behind the "every required property needs a valid default" rule below) validates each property's own sub-schema in isolation - a `$ref` pointing at a `$defs` entry declared elsewhere in the same schema block can't resolve in that isolated context and previously crashed the whole server outright (now downgraded to the same graceful exclude-with-a-warning path every other schema mistake gets, but still never something to reach for). If the same shape repeats across several properties or several files - an image object is the obvious case - write it out in full each time; there is no schema-sharing mechanism here, in one file or across files.
...
- `"type": "object", "format": "image"` - an image picker (browse the media library, or paste a URL) with a click-to-set focal point, instead of the raw-JSON fallback an unrecognised object would otherwise get. The object must declare `url`, `focalX`, and `focalY` properties (both floats `0`-`1`, defaulting to `0.5`/`0.5` for a centred crop) - the admin's picker writes exactly these three keys and nothing else. Render it through the `responsive-media` snippet (see "Snippets" below), never a hand-written `<img>` - the snippet emits the `data-cms-media` attribute the admin needs before it will accept a dragged-in replacement, and a bare `<img src="{{ section.settings.<field>.url }}">` renders perfectly while being silently un-droppable. A focal point only matters where the image is actually cropped (`object-fit: cover` plus a fixed size/aspect-ratio), rendered as `style="object-position: {{ section.settings.<field>.focalX | times: 100 }}% {{ section.settings.<field>.focalY | times: 100 }}%;"` - see `theme/blocks/gallery-image.liquid` in `granite-starter` for a worked example. Where the image isn't cropped (e.g. `media-text.liquid`), the focal point is still stored but has nothing to affect, which is fine.
- `"type": "object", "format": "video"` - a video picker (browse the media library, or paste a URL) for the short, silent loops that stand in for a hero image, instead of the raw-JSON fallback an unrecognised object would otherwise get. The object declares `url` and `poster` - the admin's picker writes exactly these two keys and nothing else. Render it through the `responsive-media` snippet (see "Snippets" below) rather than a hand-written `<video>`, for the same reason as `format: "image"` above; the poster is the still shown before the clip has loaded and whenever it will not play at all, so a loop without one is a blank rectangle on exactly the connections that can least afford to wait. Only `.mp4` and `.webm` can be uploaded, under the same 10MB cap as any other media file - long-form video belongs on YouTube or Vimeo, embedded as an iframe, not uploaded here. Deliberately no focal point, unlike `format: "image"`: a loop is played as a background rather than cropped around a subject, and a focal point that silently did nothing would be worse than its absence.
...
- `"type": "array", "items": { "type": "string" }` - no `format` opt-in needed or read here, same as `enum` above: there's only one sensible widget for a list of short strings, so the admin renders it as a repeatable list of text lines with add/remove/drag-to-reorder controls. `minItems`/`maxItems` (on the array itself) bound how many lines can be added/removed from the admin UI; `minLength`/`maxLength` (on `items`) apply to each line the same way they would on a plain string field. An `items` shape other than a plain string (an object, a nested array, or missing `type` entirely) isn't a recognised widget - it falls through to the raw-JSON fallback like any other unrecognised shape. A worked example - a multi-line animated headline - looks like `"heading": { "type": "array", "minItems": 1, "maxItems": 4, "items": { "type": "string", "minLength": 1 }, "default": ["New section"] }`, rendered with `{% for line in section.settings.heading %}<span>{{ line }}</span>{% endfor %}`.
- `"type": "array", "items": { "type": "object", "format": "image" }` - a gallery: a grid of image thumbnails with add-via-media-picker, remove, and drag-to-reorder. Each item is exactly the same `{ "url": "...", "focalX": 0.5, "focalY": 0.5 }` shape a lone `format: "image"` field stores - **no other properties on a gallery item are supported**. `minItems`/`maxItems` bound how many images the admin UI allows.
...
A page's own built-in `author`, `publishDate`, and `tags` fields (all optional - see `page.schema.json`) are indexed automatically whenever present, with no `"api": true` needed anywhere - there is no distinct "post" content type, so this is presence-based, not gated on any particular `type` value. `publishDate` is stored numerically (as above), so `GET /search.json?pageType=blog-article&sort=-publishDate` returns the newest pages of that type first - the basis for a paginated blog listing, where `blog-article` is just whatever `type` string a theme's own template happens to use, not a reserved keyword. A page's own theme-flagged section/block settings are indexed independently of these built-in envelope fields.

`GET /search.json` needs no bearer token at all - unlike every route under `/v1/`, it isn't part of that versioned admin/integration surface in the first place, a stable public contract instead. It's also read-only and only ever surfaces already-published data (unpublished pages and drafts are never indexed in the first place), so there's nothing in its response a site visitor couldn't already see by browsing the live site directly. That's deliberate: it means a section's own client-side JavaScript can call it directly with a plain `fetch()`, same-origin, no token to manage or accidentally leak. See `theme/sections/search-demo.liquid` in `granite-starter` for a worked example - a real search box wired up to this endpoint end to end.

The underlying index rebuilds itself in the background - after every publish/unpublish/delete/move/batch write, and once at boot if no index file exists yet (it's never git-tracked, so a fresh clone starts with none). `POST /v1/search/rebuild` (token-authenticated, under `/v1/`) still exists for forcing one manually, but nothing depends on ever calling it.
...
### The `responsive-media` snippet, and the drag-and-drop contract

Every scaffolded site ships `snippets/responsive-media.liquid`. Render every content image and video through it:
...
It puts `data-cms-media="<field>"` on the element it renders, plus `data-cms-image` for images. **That attribute is the entire contract for drag-and-drop replacement**: the admin searches the previewed page for it and uses its value to decide which setting a dropped file's URL is written into, so `field` must match the schema property name exactly. Markup written without it renders correctly and is simply never droppable - a silent gap, because nothing about the page looks wrong.

A video slot deliberately does **not** emit `data-cms-image`. An admin predating video support understands only that attribute, and would write an image-shaped value (`{ url, focalX, focalY }`) straight over the field's own `{ url, poster }` - corrupting the setting rather than ignoring the slot.

Guard the call on the url, as above. An unset image renders nothing at all rather than a placeholder: a theme cannot tell whether it is rendering for the admin preview or for the public site, so an editor-only affordance would show to real visitors. The first image is set with the Fields panel's own picker, and drag-and-drop replaces it from then on.

Do not build a `srcset` of generated variant filenames. The CMS names uploaded files by a hash of their own bytes, so a resized variant's name is not derivable from the original's URL - hand-built variant URLs point at files that do not exist, producing a page that looks perfect except at one breakpoint.
