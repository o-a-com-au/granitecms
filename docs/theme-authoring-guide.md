...
A post's own built-in `author`, `publishDate`, and `tags` fields are indexed automatically, with no `"api": true` needed anywhere - they're intrinsic to what a post is, not something a theme's own schema declares. `publishDate` is stored numerically (as above), so `GET /v1/search?pageType=post&sort=-publishDate` returns posts newest first - the basis for a paginated blog listing. Pages have no such automatic fields; only a page/block's own theme-flagged settings are ever indexed for a page.
