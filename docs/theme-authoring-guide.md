...
- `"type": "string", "format": "richtext"` - a rich-text editor instead of a plain text input.
...
Every other property is real, plain JSON Schema, validated with Ajv - there is no bespoke `color`/`select`/etc. type system beyond these two `format` conventions.
