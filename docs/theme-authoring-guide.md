...
A second, unrelated custom keyword can be added to any individual property: `"api": true` exposes that field's value through the agent's own search index, independent of full-text search - see `GET /v1/search` in the agent's HTTP API. Scalar values (`string`/`number`/`boolean`) are indexed as their own typed value; an array of scalars (e.g. a tags-like field) is indexed too, one entry per element, so a plain equality filter matches any one of them; an `object`-typed property flagged this way is silently ignored, the same way a mismatched `format` is. A `"type": "string", "format": "date"` field is stored as a numeric value (not text), so range operators (`gt`/`lt`/etc) work on it - the same conversion `publishDate` (below) already gets automatically. For example, `"price": { "type": "number", "api": true }` makes `GET /v1/search?filter=price:lt:50` return every page with at least one instance of that field under 50 - each block/section instance is indexed independently (not collapsed per page), so a page with several matching instances still only needs one to qualify. Like `format`/`swatches`/`allowedBlocks`, this is a plain, unvalidated JSON Schema keyword - Ajv ignores it entirely (`strict: false`).

A page's own built-in `author`, `publishDate`, and `tags` fields (all optional - see `page.schema.json`) are indexed automatically whenever present, with no `"api": true` needed anywhere - there is no distinct "post" content type, so this is presence-based, not gated on any particular `type` value. `publishDate` is stored numerically (as above), so `GET /v1/search?pageType=blog-article&sort=-publishDate` returns the newest pages of that type first - the basis for a paginated blog listing, where `blog-article` is just whatever `type` string a theme's own template happens to use, not a reserved keyword. A page's own theme-flagged section/block settings are indexed independently of these built-in envelope fields.

Unlike every other route in this agent's API, `GET /v1/search` needs no bearer token at all - it's read-only and only ever surfaces already-published data (unpublished pages and drafts are never indexed in the first place), so there's nothing in its response a site visitor couldn't already see by browsing the live site directly. That's deliberate: it means a section's own client-side JavaScript can call it directly with a plain `fetch()`, same-origin, no token to manage or accidentally leak. See `theme/sections/search-demo.liquid` in `granite-starter` for a worked example - a real search box wired up to this endpoint end to end.
