...
A second, unrelated custom keyword can be added to any individual property: `"api": true` exposes that field's value through the agent's own search index, independent of full-text search - see `GET /v1/search/fields` in the agent's HTTP API. Only scalar values (`string`/`number`/`boolean`) are meaningful this way; `api: true` on an `object`/`array`-typed property is silently ignored, the same way a mismatched `format` is. For example, `"price": { "type": "number", "api": true }` makes `GET /v1/search/fields?field=price&op=lt&value=50` return every page with at least one instance of that field under 50 - each block/section instance is indexed independently (not collapsed per page), so a page with several matching instances still only needs one to qualify. Like `format`/`swatches`/`allowedBlocks`, this is a plain, unvalidated JSON Schema keyword - Ajv ignores it entirely (`strict: false`).
