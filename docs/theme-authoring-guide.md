...
- `"type": "string", "format": "color"` - an `<input type="color">`, value as a hex string (e.g. `"#ff6600"`); an absent/empty value defaults to `#000000` in the widget (native colour inputs cannot be blank). Optionally add `"swatches": ["#c2410c", "#0f766e"]` (a plain array of hex strings) to also show a row of preset buttons above the native input - clicking one is just a shortcut into the same `onChange` the native input uses, so a brand palette and a fully custom colour both work through the one field. Deliberately not `"enum"`: enum is a real, Ajv-enforced constraint (anything outside the list fails validation), which would defeat the point of still allowing a custom colour - swatches are a suggestion only, same "UI hint, unvalidated" status as `format` itself.
- `"enum": [...], "format": "radio"` - the same enum shape a plain `select` uses, rendered as a group of radio buttons instead of a dropdown. Omit `format` (or use anything else) on an enum and it stays a `<select>`.
