...
| # | Criterion | Proof |
|---|---|---|
| G1 | Rebuild from a clean checkout produces an index; a basic FTS query returns the expected page for a known term | `test/search/rebuild-index.test.ts :: G1: rebuild from a clean checkout produces an index; a basic FTS query returns the expected page for a known term`. Verified against Node v26.5.0 (this environment's only available version); the build plan's own open `[REVIEW]` item asking to confirm `node:sqlite`/FTS5 behaviour on the documented `engines.node` floor (>=22.6.0) / target LTS is not resolved by this session and should not be read as closed. ~~Unresolved~~ **Resolved (2026-09-08): the floor itself was wrong.** Bisecting real Node builds found `node:sqlite` needs `--experimental-sqlite` below v22.13.0 and FTS5 support itself isn't compiled in until v22.16.0 - every version from the old `>=22.6.0` floor up to 22.15.x would have hard-crashed on a search rebuild. `engines.node` is now `>=22.16.0`; see `docs/cms-build-plan.md`'s SQLite section for the full write-up. |
